...
public class FingerPrintHelp {
    ...
    public static  void callFingerPrint(final OnCallBackListenr listener){
        ...
        if (!managerCompat.isHardwareDetected()){ //判断设备是否支持
        	...
        	listener.onSupportsuccess();
        }
        KeyguardManager keyguardManager =(KeyguardManager) App.getContext().getSystemService(App.getContext().KEYGUARD_SERVICE);
        if (!keyguardManager.isKeyguardSecure()) {//判断设备是否处于安全保护中
           Log.e("指纹", keyguardManager.toString());
        	if (listener != null)
            listener.onInsecurity();
            return;
        }else{
        	listener.onFInsecurity();
        }
...
//        Log.e("cancellationSignal", cancellationSignal+"");
